...
export default function AdminDemandes() {
  ...
  const handleStatusChange = async (demandeId, newStatus) => {
    try {
      ...
      showToast("success", `Request ${newStatus === "approuvee" ? "approved" : "rejected"} successfully!`);
  ...
  const handleDeleteAll = async () => {
    if (!confirm("Are you sure you want to delete ALL requests? This action cannot be undone.")) return;
      ...
      <div className="container mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage and respond to student document requests</p>
          </div>
          {demandes.length > 0 && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setFilterStatus("en_attente")}>
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
          ...
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setFilterStatus("approuvee")}>
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
          ...
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setFilterStatus("rejete")}>
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
        ...
        {!loading && (
          <div className="grid gap-4">
            {filteredDemandes.map((demande) => {
              ...
              return (
                <Card key={demande._id} className="overflow-hidden hover:shadow-lg transition-shadow">
                  <div className={`h-2 bg-gradient-to-r ${getStatusColor(demande.statut)}`} />
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-4 flex-1">
                        <div className={`h-12 w-12 rounded-lg bg-gradient-to-br ${getStatusColor(demande.statut)} flex items-center justify-center flex-shrink-0`}>
                      ...
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm" onClick={() => openDetailDialog(demande)}>
                          View Details
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => openDeleteDialog(demande)}>
        ...
        <Dialog open={showDetailDialog} onOpenChange={setShowDetailDialog}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>Review and respond to this request</DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-4 py-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-base font-semibold">{selectedDemande.nom}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Type</p>
                    <p className="text-base font-semibold">{getTypeLabel(selectedDemande.type)}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Student</p>
                    <p className="text-base font-semibold">
                      {selectedDemande.etudiant?.prenom} {selectedDemande.etudiant?.nom}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Email</p>
                    <p className="text-base font-semibold">{selectedDemande.etudiant?.email}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Status</p>
                    <Badge className={`${getStatusBadge(selectedDemande.statut).style} border`}>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowDetailDialog(false)} disabled={processing}>
                Cancel
              </Button>
              {selectedDemande?.statut === "en_attente" && (
                <>
                  <Button
                    variant="destructive"
                    onClick={() => handleStatusChange(selectedDemande._id, "rejete")}
                    disabled={processing}
                  >
                  ...
                  <Button
                    onClick={() => handleStatusChange(selectedDemande._id, "approuvee")}
                    disabled={processing}
                  >
            ...
            {selectedDemande && (
              <div className="py-4">
                <Alert>
                  ...
                  <AlertDescription>
                    <div className="font-semibold mb-1">{selectedDemande.nom}</div>
                    <div className="text-sm text-muted-foreground">
                      Student: {selectedDemande.etudiant?.prenom} {selectedDemande.etudiant?.nom}
                    </div>
                  </AlertDescription>
                </Alert>
              </div>
